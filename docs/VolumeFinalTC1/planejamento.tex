...
Planejamento automatizado é uma sub-área da inteligencia artificial que estuda o processo de geração automática de planos.\frm{Esta frase não poderia ser um pouco mais circular?} 
O processo para gerar um plano, é feito a partir da escolha e organização das ações, antecipando os resultados esperados das ações, em busca do seu objetivo. Um plano pode ser descrito como uma sequencia de ações que se forem executadas chegam a um objetivo \cite{ghallab2004automated}. O planejamento na computação se diferencia das outras áreas pelo fato de que todo o plano é gerado automaticamente \cite{intelligence2003modern}.\frm{Repetição de pontos semelhantes, e introdução de jargão sem explicação. Cada a conexão com busca?} 
...
Com a descrição do problema os estados e transição não precisam ser explícitos, mas faz com que os estados não descritos possam ser computados \cite{ghallab2004automated}.\frm{Duas últimas frases são uma mistura de erros com contradições. Primeiro tu não precisa representar os estados e transições, depois isto é implícito, mas não pode ser computado??!?! O que é isto companheiro?} 
Como foi visto no Capitulo~\ref{chap:agentes}, os estados são os estados para o agente se situar no ambiente e transições são para a interação com o ambiente. \frm{Mas tu não viste NADA DISTO no capítulo de agentes!!!}
Um problema de planejamento pode ser descrito como \textit{P} = \textit{($\Sigma$, $s_{0}$, g)}. 
Onde \cite{ghallab2004automated}:\frm{Onde é uma frase inteira?!?!}
...
Vamos lembrar do problema do Capitulo \ref{chap:busca}, chegar a uma determinada cidade, estados para esse problema podem ser estar em determinada cidade e ter ligação entre as cidades, representado respectivamente como \textit{estar(cidade)} \textit{terLigação(cidadeOrigem, cidadeDestino)}. \frm{Parse error}

Além dos estados do ambiente precisamos determinar as transições, que alteram os estados. \frm{Repetição, que tu não definiste, se não me falha a memória.}
...
No nosso exemplo, um operador de planejamento seria\frm{Seria ou é?} a mudança de uma cidade $A$ para outra cidade $B$, que poderia ser representado como:
\begin{itemize}
...
O nome deve ser único pelo proposito de o nome poder se referir ao operador por completo\frm{?!?!}, ou seja, após definido apenas com o nome pode-se inferir as pré e pós condições, assim escrevendo o nome(\textit{op}) para se referir a todo o operador de planejamento \textit{op} \cite{ghallab2004automated}. 

O processo de geração do plano é feito pelo planejador, para isso ele utiliza a representação do problema, o estado inicial e os objetivos como entrada \cite{ghallab2004automated}.\frm{?!?!} A figura \ref{fig:planmodelo} \underline{representa} esse processo. 
...
O domínio do problema é a parte do mundo que é expressada pelo conjunto de informações que é usado pelo planejador para gerar o plano \cite{intelligence2003modern}.\frm{E agora é o momento de dizer em uma sentença, o que é o domínio?} 
...
Dentro da área de planejamento existe o planejamento hierárquico\frm{Não seja tão abruto para entrar em HTN. Qual a limitação de panejamento clássico? O que poderia ajudar nisto? O que é conhecimento de domínio? Por que usá-lo?}, chamado de \textit{Hierarchical Task Network} (HTN). 
Em planejamento HTN as ações são tratadas em mais alto nível \cite{intelligence2003modern}. 
Essa abordagem de planejamento cria planos decompondo tarefas em tarefas ainda menores \cite{ontanon2015adversarial}.\frm{Tu tentaste resumir HTN em três frases! Mas tu tens que ir adicionando os tópicos.}

Uma tarefa $t$ é uma representação de uma atividade no ambiente. Existem dois tipos de tarefa \cite{intelligence2003modern}\frm{Compressão máxima, compreensão mínima pelo leitor}: 
...
Para decompor as tarefas não-primitivas em primitivas é necessário um conjunto de métodos \cite{ghallab2004automated}. 
Um método é composto por $m = (t, C, w)$. \frm{????}
...
\frm[inline]{Daqui para adiante é óbvio que tu escreveste na corrida. Revisa sob a luz dos comentários anteriores e me avisa quando terminaste.}
