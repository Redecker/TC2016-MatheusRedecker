...
\begin{itemize}
	\item estados, um conjunto de predicados, que assumem o valor de verdadeiro ou falso dependendo da situação do ambiente;\frm{Isto é um conjunto de fragmentos de sentença, mas certamente não é uma frase coerente. Note, as definições (citando), to \textbf{pode} levantar de outro trabalho!!} 
...
Um exemplo de tarefa primitiva é o exemplo da seção anterior\frm{Qual deles?}, de mover um objeto de um lugar para o outro, pois altera o estado do ambiente, denotado por $(move~ ?from~ ?to)$. 
...
A representação das tarefas \underline{são feitas} como em planejamento clássico, sendo as tarefas primitivas a função que tinha as ações \cite{meneguzzi2015planning}.\frm{Tentando comprimir idéias de novo né? ``A representação das tarefas \textbf{são feitas} como em planejamento clássico''??? Tu acabaste de dizer que não é a mesma coisa?!?! Tu queres dizer que só tarefas primitivas \textbf{correspondem} a ações executáveis análogas a planejamento clássico, né.}  
...
Um problema de planejamento\frm{HTN?} $P$ é definido como $P = (d, I, D)$, onde $D$ é um domínio, $d$ é a tarefa de ligação inicial e $I$ é um estado inicial como no planejamento clássico. 
...
Um método tem um filho\frm{O que é um filho aqui?} para cada uma das tarefas presentes na tarefa de ligação\frm{O que é uma tarefa de ligação?}. 
Tarefas primitivas não podem ter filhos\frm{Pobrezinhas... Tu não queria dizer que não podem ser decompostas?}. 
...
O algoritmo de \textit{Total-order Forward Decomposition} (TFD) gera um plano a partir de uma rede de tarefas inicial com ordenação total, como detalhado no  Algoritmo~\ref{alg:tfd}.\frm{Não fica bem melhor na voz ativa? Assim?} 
...
O Algoritmo~\ref{alg:ahtn}~\cite{ontanon2015adversarial} é a representação da técnica de AHTN, e assume que existem dois jogadores, MAX e MIN, como no algoritmo de \textit{minimax search} apresentado no Capitulo~\ref{chap:busca}.\frm{Referências nomeadas, sempre em maiúsculo.} 
O algoritmo também assume uma busca em uma arvore com uma máxima profundidade $d$.\frm{Não lembro agora, mas é bom que tu tenha mencionado isto lá no Capitulo~\ref{chap:busca}}

\frm[inline]{Utilize as linhas do algoritmo (E.g. a primeira linha que eu referencio é a Linha~\ref{alg:lin:firstLine}), para tu dar uma idéia do que tu estás falando no algoritmo. Antes de começar a ir nos símbolos, tu tens que descrever em um parágrafo a intuição do algoritmo.}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]		
		...
		\State \Return $\Call{AHTNMin}{(\gamma(s,t), N_{+}, N_{-}, t, t_{-}, d-1)}$ % FRM - Note como tu faz uma chamada de subfunção
		\EndIf
...
Na raiz da arvore, $t_{+} = \perp$ e $t_{-} = \perp$ indicam que nenhuma ação ainda foi executada~\cite{ontanon2015adversarial}.\frm{E o que isto quer dizer?}

A função \textit{nextAction(N,t)} faz com que, dado um HTN $N$ e um ponteiro $t$\frm{Utilize o math mode no meio do texto para indicar símbolos matemáticas}, seja encontrada a tarefa primitiva que deve ser executada em $N$ depois da tarefa $t$. Se $t = \perp$ então é retornado a primeira tarefa primitiva a ser executada em $N$. 
Se $N$ ainda não estiver completamente decomposto, ou seja, ainda existem tarefas não primitivas, e existe nenhuma tarefa primitiva em $N$ então \textit{nextAction(N,t)} = $\perp$ \cite{ontanon2015adversarial}. \frm{Eu estou bem perdido aqui. Seria legal tu usar as linhas do algoritmo para indicar do que tu estás falando...}
...
A partir destas definições, o algoritmo para AHTNMin é análogo\frm{Análogo a que?}. 
...
Uma vez que o jogador MAX\frm{Decide como tu vais representar o Max e o Min no texto, e SEJA CONSISTENTE!! Eu sugiro \textsc{Max} e \textsc{Min}} decompõe sua HTN para o ponto onde a primeira ação pode ser gerada(nodo $n_{2}$ e $n_{5}$), é o turno de MIN de decompor. Quando MIN pode gerar suas ações, a profundidade máxima foi atingida (nodos $n_{3}$ e $n_{4}$). 
