...
A utilidade é definida para ser a soma esperada das recompensas obtidas seguindo a politica $\pi$. Podendo ser representado pela equação \ref{eq:utility}, onde $R(s)$ é a recompensa do estado $s$, $S_{t}$ é o estado alcançado no tempo $t$ quando executado a politica $\pi$ e $S_{0} = s$. É preciso de um fator de desconto $\gamma$ para descrever a performasse do agente para as recompensas atuais sobre as futuras, este fator é um numero entre 0 e 1, quando 1 as recompensas são exatamente equivalentes as recompensas futuras. \cite{intelligence2003modern}.

\begin{equation}
\label{eq:utility}
	U^{\pi}(s) = E 
	\begin{bmatrix}	
	\displaystyle\sum_{t=0}^{\infty} \gamma^{t} R(S_{t})
	\end{bmatrix}
\end{equation}
...
Como o \textit{Q-learning} não precisa do modelo de transição $P(s' | s, a)$ ele é chamado de um método livre de modelo. A equação \ref{eq:qler2} representa o calculo do valor de $Q(s, a)$. O algoritmo \ref{alg:qlearning} apresenta o método de \textit{Q-learning} para um agente \cite{intelligence2003modern}.
