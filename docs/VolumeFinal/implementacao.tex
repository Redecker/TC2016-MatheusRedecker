...
\frm[inline]{Reorganizar isto. Pensa em um estrutura de alto nível: unidades, recursos, construções ou algo assim. Tu estás indo de um para o outro para um terceiro, de volta pro segundo...}
...
Cada método pode chamar outros métodos.\frm{?? Organize...}
...
O algoritmo de AHTN requer uma função de avaliação para determinar o valor de utilidade para cada estado do jogo.\frm{Organiza!! Neste momento seria bom tu dizer que tu criaste duas heurísticas, antes de ir falando de uma ou de outra}
...
As unidades são ponderadas pelo custo de treinamento ou de construção.\frm{Tu tens uma formula, então por que não formalizar numa formulinha?}
...
Antes de iniciar a implementação do algoritmo de AHTN, é preciso criar os métodos que executam as ações dentro do MicroRTS.\frm{Encontrar alguma maneira de diferenciar métodos HTN de métodos em Java (como por exemplo dizer: precisamos criar métodos em Java para executar as ações geradas pelo JSHOP dentro do MicroRTS...)} 
...
O JSHOP2 gera duas classes java, uma representando o domínio, e outra representando o problema.\frm{E aqui tu repete isto...}
...
A partir dessa análise houve o mapeamento do estado do jogo para os predicados do problema.\frm{Faz um diagraminha desta comunicação do MicroRTS com o JSHOP, e te refere a isto}
...
\frm[inline]{Eu não entendi o que tu queres me dizer aqui. Se lembrar dos métodos na decomposição é importante então? Ou só o plano serve?}
...
Na linha~\ref{alg:meuahtn:nexaction}\frm{Linha, Algoritmo, Figura (tudo com ref) tem que ser em maiúscula} o algoritmo aplica a próxima ação primitiva do plano no estado.
