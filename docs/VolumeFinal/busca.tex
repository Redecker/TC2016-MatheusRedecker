...
Com o intuito de encontrar uma sequência de ações que um agente consiga alcançar o seu objetivo, é possível utilizar técnicas de busca.\frm{Não ficaria melhor: Técnicas de busca visam encontrar sequências de ações para um agente alcançar um objetivo. } 
O princípio da busca é encontrar uma solução de um problema através de um conjunto de ações que alcancem o objetivo desejado.\frm{Mas esta sentença é meio replicação da anterior, planeje o que tem que ser dito.} 
Para utilizar as técnicas de busca é preciso formalizar o problema a ser resolvido e o objetivo a ser alcançado. 
Para utilizar as técnicas de busca, como entrada é recebido um problema e como resolução é retornado um conjunto de ações. \frm{Para utilizar as técnicas de busca... Para utilizar as técnicas de busca... É um, ou outro, ou os dois?}
...
\begin{itemize}
	...
	\item custo do caminho, uma função que define um valor numérico para cada ação realizada em um estado. Essa função pode ser denotada por $c(s, a, s')$, onde s é o estado atual do agente, a é a ação que será aplicada ao estado s e $s'$ é o estado resultante aplicando o modelo de transição resultado(s, a).\frm{Este último ponto parece meio estranho, é isto que está no Norvig mesmo?} 
\end{itemize}   


Esses elementos são necessários como entrada para que as técnicas de busca consigam ser utilizadas.\frm{Terceira vez que tu escreve isto...} 
As técnicas de busca visam encontrar a solução para esse problema, ou seja, a sequência de ações que leve ao objetivo. \frm{Mais uma vez...} 
...
Existem diferentes técnicas para encontrar uma solução.\frm{No primeiro parágrafo busca era uma técnica, mas agora existem diferentes técnicas? Ou tu queres dizer \textbf{estratégias de busca}?} 
...
Com a finalidade de exemplificar um problema de busca, considere o mapa apresentado na figura \ref{fig:mapabusca}.\frm{Como o Yoda, não é legal falar.} 
Cada círculo representa uma cidade, e as linhas entre as cidades são estradas que ligam as cidades uma a outra. 
A única ação possível é a de se locomover entre as cidades que tenham ligação.\frm{Isto é uma única ação ou várias?} 
...
A busca adversaria é utilizada nos jogos que estão situados em ambientes competitivos e de multi agentes.\frm{É utilizada sempre? Pode ser utilizada?} 
Em um jogo o jogador, preferencialmente, não informa suas jogadas previamente, assim tornando o ambiente imprevisível.\frm{O jogador é previsível, é exatamente isto que tu estás fazendo com o minimax, prevendo que opções ele tem!!} 
Os objetivos dos jogadores entram em conflito, pois ambos estão em busca da vitória.\frm{Explique isto melhor... Tu poderia falar do resultado do (soma) do jogo para ilustrar isto.} 
...
Os componentes devem se adequar ao ambiente competitivo.\frm{Não sei o que tu queres dizer aqui.} 
...
A árvore representa em cada nodo um estado do jogo e em cada ligação os estados resultantes após a execução de cada ação possível para o estado.\frm{Esta frase está meio estranha?} 
...
A Figura~\ref{fig:gametree} ilustra esse processo.\frm{Ilustra o que? Isto parece uma árvore de jogo também, mas tem outros númerinhos que tu não explica aqui...}
...
Existem algumas abordagens que utilizam busca adversaria com um nível de abstração mais alto para tentar minimax esse problema~\cite{ontanon2013survey}. \frm{E alpha-beta pruning?}