...
A Figura~\ref{fig:sequencia} ilustra o diagrama de sequência do funcionamento da classe\frm{Mas tu não pode ter um diagrama de sequencia da classe toda, estes diagramas são de métodos na classe, então suponho que seja do main?} \textit{GameVisualSimulation}. 

\todo[inline]{Felipe, pode ler daqui em diante. Obrigado}

O algoritmo Algoritmo~\ref{alg:jogo} apresenta o pseudo código da classe. \frm{Idem} 
...
Nas linhas~\ref{alg:jogo:issue1} e \ref{alg:jogo:issue2} as jogadas são testadas para que não haja nenhuma violação quanto as restrições do jogo.\frm{Acho que isto merece um pouco mais de explicação.}
...
\frm[inline]{O trecho acima não está claro, esta biased está biased para uma destas três? Ou biased para algum tipo de estratégia, três das quais, são estas?}
Ainda dentre as estratégias Hard-Coded, existem técnicas que utilizam uma mesma dinâmica.\frm[inline]{Isto também não está tão claro, seria legal tu tentar organizar isto. Algo nesta linha:\\
Estratégias hard-coded consistem em um certo tipo de script com um objetivo inflexível definido em tempo de programação...
Para jogos RTS, existe um conjunto bem conhecido deste tipo de estratégia: X, Y, Z...
Existem diversas maneiras de implementar estratégias hardcoded para dar a ilusão de variabilidade: Random e RandomBiased...
RandomBiased escolhe entre ...
} 
...
Como é o caso das estratégias Minimax, Monte Carlo, e Portfolio Search.\frm{Citações para estes trabalhos.}
...
O JSHOP2 foi desenvolvido em Java por Dana Nau e sua equipe de pesquisa. \frm{Certeza que foi o Nau? Pq a tua referência tem outro nome. Acho que tem um paper certo para citar que não é a documentação do JSHOP}
 %
 % 
O JSHOP2 recebe como entrada uma descrição do domínio e um problema de planejamento.\frm{Faça a ligação disto com o que tu já explicaste de HTN na Seção~\ref{sec:htnPlanning} e na Seção~\ref{chap:planejamento}. Numeração de seções não é só para bonito.} 
...
Um exemplo de operador é apresentado a seguir:\frm{Explica o que é isto... comentários...}

\frm[inline]{Listing te permite definir palavras reservadas e estilos de comentários, olha isto e adiciona algo para o JSHOP, fica bem mais bonito}
...
O domínio completo\frm{Qual domínio completo fica assim? Isto está bem superficial} começa com o nome do domínio e em seguida os operadores e métodos descritos. O domínio completo exemplo ficaria assim: 
...
\frm{Estilos diferentes (math mode para listing no texto inline). Se tu não quer aprender a usar listing inline, pelo menos usa texttt, tipo \texttt{path ?a ?b} }
...
\frm{Isto das classes java é realmente relevante? Se tu não vais explorar isto então não fala... Talvez tu vá explorar isto mais tarde na implementação (então fala lá)}
O JSHOP2 gera duas classes java, uma representando o domínio, e a outra representando o problema.
A classe do problema gera o plano, utilizando as informações da classe da descrição do domínio.\frm{Descrição muito \textbf{mágica} da coisa, explica isto melhor}
...
O plano indica o custo do plano\frm{O plano indica o custo do plano??}, junto com os operadores em ordem que devem ser executados para que o objetivo seja alcançado. 
Caso existam mais planos que levem ao mesmo objetivo, o JSHOP2 apresenta os planos na mesma estrutura.

\frm[inline]{Por que eu estou lendo a descrição do capítulo de novo aqui?}
